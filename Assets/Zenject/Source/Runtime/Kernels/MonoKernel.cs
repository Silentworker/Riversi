...
namespace Zenject
{
    public abstract class MonoKernel : MonoBehaviour
    {
        ...
        {
            Log.Debug("DependencyRoot ({0}): StartGame called, Initializing IInitializable's", this.GetType().Name());
        ...
        {
            // _disposablesManager can be null if we get destroyed before the StartGame event
