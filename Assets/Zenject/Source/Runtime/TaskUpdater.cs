...
namespace Zenject
{
    ...
    public class TickablesTaskUpdater : TaskUpdater<ITickable>
    {
        protected override void UpdateItem(ITickable task)
        {
            ...
            using (ProfileBlock.ResetGame("{0}.Tick()".Fmt(task.GetType().Name())))
#endif
            {
                task.Tick();
            }
    ...
    public class LateTickablesTaskUpdater : TaskUpdater<ILateTickable>
    {
        ...
        {
#if PROFILING_ENABLED
            using (ProfileBlock.ResetGame("{0}.LateTick()".Fmt(task.GetType().Name())))
#endif
            {
                task.LateTick();
            }
    ...
    public class FixedTickablesTaskUpdater : TaskUpdater<IFixedTickable>
    {
        ...
        {
#if PROFILING_ENABLED
            using (ProfileBlock.ResetGame("{0}.FixedTick()".Fmt(task.GetType().Name())))
#endif
            {
                task.FixedTick();
            }
